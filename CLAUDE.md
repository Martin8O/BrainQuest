...
## 1. What this is
**BrainQuest** — a Next.js learning app that reads a read-only cross-project Obsidian
vault (`<vault>`) and turns it into an active learning game: spaced-repetition review,
a dependency-ordered **skill tree** with mastery/progress, a daily session, and an AI tutor. Goal: Martin
*absorbs* the 82 learning notes + 93 concepts fast, durably, and with momentum.
**Building it is itself his next learning project** (see §11).
...
- **This repo** `D:\Projekty\BrainQuest` → GitHub
  `https://github.com/Martin8O/BrainQuest.git` (repo + local folder previously renamed; old URL still redirects).
- **`local/`** (gitignored) — the ONLY private home: Claude's memory, bootstrap, plan, prompt standard, scratch.
- **`<vault>`** — the CONTENT SOURCE (read-only Obsidian vault). App reads it; the teaching layer writes
...
- **`data/`** — runtime app review state + progress. `data/*.json` is **gitignored** (personal, per-device;
  never shipped). **Secrets/API keys → `local/` only.**
...
## 5. Stack & conventions
- **Next.js + TypeScript** (App Router) + Tailwind. Reuses web skills from a sibling web project.
...
## 10. Hard rules
- `local/` NEVER goes to git. `data/*.json` runtime state is **gitignored** (personal progress; never shipped).
  No secrets/keys/`.env`/`node_modules` committed. Scope every commit (`git status` first; no blind `git add -A`).
- Teaching-layer writes to the vault are free (§11); avoid wholesale refactors of unrelated notes without a heads-up.
...
Martin is a motivated **learner** working toward an AI/tech career; the build must teach him. Governed by
